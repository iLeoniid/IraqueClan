...
public class ClanPromoteCommand implements gg.leo.IraqueClan.clan.ClanSubCommand {
    private final IraqueClan plugin;

    public ClanPromoteCommand(IraqueClan plugin) {
        this.plugin = plugin;
    }

    @Override
    public void execute(Player player, String[] args) {
        if (!player.hasPermission("iraqueclan.promote")) {
        ...
        if (args.length < 2) {
            player.sendMessage(this.plugin.getConfigManager().getPrefixedMessage("usage-promote"));
            return;
        }
        ...
        boolean promoted = this.plugin.getClanManager().promoteMember(player.getUniqueId(), target.getUniqueId());
        if (promoted) {
            ClanRole newRole = clan.getMemberRole(target.getUniqueId());
            player.sendMessage(this.plugin.getConfigManager().getPrefixedMessage("clan.promoted")
                    .replace("{player}", target.getName())
                    .replace("{role}", newRole != null ? newRole.getDisplayName() : "Membro"));
            target.sendMessage(this.plugin.getConfigManager().getPrefixedMessage("clan.you-were-promoted")
                    .replace("{role}", newRole != null ? newRole.getDisplayName() : "Membro"));
        } else {
            player.sendMessage(this.plugin.getConfigManager().getPrefixedMessage("clan.cannot-promote-higher"));
        }
    }
}
